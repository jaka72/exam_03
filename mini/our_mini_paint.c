...

{
	...
	if (scan_ret == 3)
	{
		...
		{
			//perror("Added, error from get_info, invalid widht or height");
			return (1);
		}
		...
		if (!drawing->matrice)
		{
			//perror("Added, error from get_info, if no drawing matrice");
			return (1);
		}
		i = 0;
		...
		//printf("bckg: [%c]\n", background);
		//perror("No error, it did not detect float instead of char?? ");
		return (0);
	}
	//perror("Added, error from get_info, before last return");
	return (1);
}
...
int print_info(drawing *drawing)
{
	...
	{
		//printf("Q");  // WHY DOES HE HAVE THIS HERE ???
...
int execute(FILE *file)
{
	...
	if (!get_info(file, &drawing))
	{
		...
		//printf("Begin get_info: scanret: %d \ntype: [%c]\n", scan_ret, circle.type);
		//printf("circle.x  [%f], circle.y  [%f]\n", circle.x, circle.y);
		//printf(" circle.radius [%f], circle.color [%c])\n",  circle.radius, circle.color);

		while (scan_ret == 5)
		{
			if (apply_op(&circle, &drawing))
			{
				//perror("Added, error from apply_op");
				return (1);
			}
			scan_ret = fscanf(file, "%c %f %f %f %c\n", &circle.type, &circle.x, &circle.y, &circle.radius, &circle.color);
			//print_info(&drawing);
		...
		{
			print_info(&drawing);
			//perror("Added, error from scan_ret == -1 ??????? Now should return 0 ????");
			return (0);
		}
		//perror("Added, error loop, scan ren != 5");
		return (1);
	}
	//perror("Added, error from get_info");
	return (1);
}
